...
export default function VCardClient({ showCatalog = false }: VCardClientProps) {
    ...
    if (data.status === 'cancelado') {
        if (typeof window !== 'undefined') {
            // Force redirect to ActivaQR's master profile/catalog instead
            const destination = showCatalog ? '/catalog/activaqr-9ag4' : '/card/activaqr-9ag4';
            window.location.replace(destination);
        }
        return null;
    }

    // PANTALLA DE VALIDACIÓN SI ES PENDIENTE (SOLO PARA PLAN CATÁLOGO)
    /* 
    if (data.status === 'pendiente' && data.plan === 'catalog') {
        return (
    ...
    const handleHeroClick = () => {
        ...
        if (action === 'wifi') {
            ...
            if (data.hero_file_url) {
                ...
                // If it's Base64, we should provide an extension based on the MIME type
                if (data.hero_file_url.startsWith('data:')) {
                    ...
                    // For external URLs, we try to force download if possible
                    a.download = `archivo_${data.nombre.replace(/\s/g, '_')}`;
                }
                ...
                // FALLBACK: If no file uploaded, download contact (VCF)
                downloadVCF();
            }
        } else if (action === 'link') {
        ...
        <div className="relative" style={{ 
            ...
            <div className="max-w-6xl mx-auto relative z-10 pt-4 md:pt-8 px-2 md:px-4">
                <div className="grid lg:grid-cols-12 gap-6 md:gap-8 items-stretch">
                    {/* Nested Grid to wrap Bio/Solutions and Sidebar together (limits sticky) */}
                            ...
                            <div className="flex flex-col md:flex-row gap-8 md:gap-14 items-center md:items-start relative z-10 w-full">
                                ...
                                <div className="text-center md:text-left flex-1 min-w-0 w-full">
                                    ...
                                    <style jsx>{`
                                        ...
                                        .video-container-tiktok :global(iframe) {
                                            aspect-ratio: 9/16;
                                        }
                                                    ...
                                                    return (
                                                        <>
                                                            {visibleProducts.map((line: string, index: number) => {
                                                                ...
                                                                // Improved emoji extraction: match any emoji (including variants/zwj) at the start of the line or after simple bullets
                    ...
                    {/* Star Rating Section (Social Proof) - BEFORE CANALES ON MOBILE */}
                    {(data.plan === 'business' || data.plan === 'catalog') && data.google_rating && (
                        <div className="lg:col-span-12 order-2 xl:order-4 mt-8 md:mt-16">
                            ...
                            >
                                {/* Decorative glow - uses theme color */}
                                <motion.div 
                    ...
                    {/* Mobile Only: Contact Details (At the end) */}
            ...
            <AnimatePresence>
                {!isLightboxOpen && !isFooterVisible && (
                    <motion.div 
