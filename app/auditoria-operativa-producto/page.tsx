...
import { 
  ArrowLeft, ArrowRight, Play, ShieldCheck, Camera, Smartphone, User, Mail
} from "lucide-react";
...
export default function AuditoriaMicroFunnel() {
  ...
  return (
    <main className={`bg-[#0a0a0a] text-white font-sans selection:bg-[#FF6B2B] h-screen overflow-hidden relative ${step >= 3 ? "overflow-y-auto" : ""}`}>
      ...
      {/* Botón Volver (Sutil) */}
      <button 
        onClick={() => step > 1 ? setStep((s) => (s - 1) as Step) : window.location.href = "/auditoria-operativa"}
      ...
      <StepWrapper isVisible={step === 1}>
        <div className="absolute inset-0 z-0">
          <motion.div 
            ...
            style={{ backgroundImage: "url('/images/Reingenierìa/portada_laptop_supermercado.webp')" }}
          />
        ...
        <div className="container mx-auto px-12 md:px-20 text-left max-w-6xl relative z-10 pt-20">
          ...
          >
            Activar mi Auditoría <ArrowRight size={24} />
          </button>
        </div>
      </StepWrapper>
      ...
      <StepWrapper isVisible={step === 2}>
        <div className="container mx-auto px-6 text-center max-w-4xl py-20">
          <p className="text-white/40 text-[10px] font-black uppercase tracking-widest mb-4">
            Cómo la Auditoría ActivaQR2 protege tu rentabilidad.
          </p>
          <h2 className="text-2xl md:text-4xl font-black mb-12 tracking-tighter uppercase">Implementación Estratégica Incluida</h2>
          <div className="relative aspect-video bg-[#111] rounded-2xl overflow-hidden border border-white/5 shadow-2xl mb-10">
            <iframe 
              src="https://iframe.mediadelivery.net/embed/261314/9278c314-648f-4220-b1d4-b2c94535ffa8?autoplay=true&loop=false&muted=false&preload=true&responsive=true" 
              loading="lazy" 
          ...
          <button onClick={() => setStep(3)} className="inline-flex items-center gap-3 bg-[#FF6B2B] text-white font-black text-base px-12 py-5 rounded-xl hover:scale-105 transition-all">
            Ir al pago de suscripción <ArrowRight size={20} />
          </button>
        </div>
      </StepWrapper>
      ...
      {step === 3 && (
        ...
        >
          <div className="w-full max-w-5xl">
            <RegisterWizard paymentFirst={true} initialPlan="auditoria" />
          </div>
        </motion.div>
      )}
