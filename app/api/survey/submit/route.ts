...
export async function POST(request: NextRequest) {
    try {
        ...
        const query = `
            INSERT INTO registraya_encuesta_respuestas 
            (p1, p2a, p2b, p3, p4, p5, p6, total_score, color_semaforo, p3_resp_custom, nombre_local, user_metadata)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        ...
        const values = [
            p1, p2a, p2b, p3, p4, p5, p6,
            total_score, color_semaforo, p3_resp_custom,
            nombre_local, user_metadata ? JSON.stringify(user_metadata) : null
        ];
